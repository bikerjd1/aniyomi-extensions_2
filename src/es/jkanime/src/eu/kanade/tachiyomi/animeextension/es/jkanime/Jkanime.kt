...
class Jkanime : ConfigurableAnimeSource, ParsedAnimeHttpSource() {
    ...
    // Popular Anime
    override fun popularAnimeSelector(): String = "div.anime__item"
    ...
    override fun popularAnimeFromElement(element: Element): SAnime {
        return SAnime.create().apply {
            ...
            thumbnail_url = element.select("div.anime__item__pic").attr("data-setbg")
    ...
    override fun latestUpdatesFromElement(element: Element): SAnime {
        return SAnime.create().apply {
            setUrlWithoutDomain(element.attr("abs:href").substringBeforeLast("/").substringBeforeLast("/"))
    ...
    override fun searchAnimeParse(response: Response): AnimesPage {
        ...
        val hasNextPage = document.select("a.nav-next").isNotEmpty()
        val animeList = if (document.select(".col-lg-2.col-md-6.col-sm-6").isNotEmpty()) {
            document.select(".col-lg-2.col-md-6.col-sm-6").map { animeData ->
                SAnime.create().apply {
                    title = animeData.select("div.anime__item #ainfo div.title").text()
                    thumbnail_url = animeData.select("div.anime__item a div.anime__item__pic").attr("data-setbg")
                    setUrlWithoutDomain(animeData.select("div.anime__item a").attr("href"))
                    status = parseStatus(animeData.select("div.anime__item div.anime__item__text ul li:nth-child(1)").text())
                    genre = animeData.select("div.anime__item div.anime__item__text ul li").joinToString { it.text() }
                }
            }
        } else {
            document.select(".card.mb-3.custom_item2").map { element ->
                SAnime.create().apply {
                    setUrlWithoutDomain(element.select(".custom_thumb2 > a").attr("abs:href"))
                    title = element.select(".card-title > a").text()
                    thumbnail_url = element.select(".custom_thumb2 a img").attr("abs:src")
                    description = element.select(".synopsis").text()
                }
            }
        }
    ...
    override fun animeDetailsParse(document: Document): SAnime {
        ...
        anime.thumbnail_url = document.selectFirst("div.anime__details__pic.set-bg")?.attr("data-setbg") ?: ""
        anime.title = document.selectFirst("div.anime__details__title h3")?.text() ?: ""
        anime.description = document.selectFirst("div.anime__details__text p")?.ownText() ?: ""

        document.select("div.anime__details__widget ul li").forEach { animeData ->
            val data = animeData.select("span").text()
            when {
                data.contains("Genero") -> {
                    anime.genre = animeData.select("a").joinToString { it.text() }
                }
                data.contains("Estado") -> {
                    anime.status = parseStatus(animeData.text())
                }
                data.contains("Studios") -> {
    ...
    override fun episodeListParse(response: Response): List<SEpisode> {
        ...
        val animeId = pageBody.select("div#guardar-anime.btn.btn-light.btn-sm.ml-2")
            .attr("data-anime")

        val lastEp = client.newCall(GET("$baseUrl/ajax/last_episode/$animeId/")).execute()
            .asJsoup().body().text()
        ...
        for (i in 1..lastEp) {
            episodes.add(
                SEpisode.create().apply {
                    setUrlWithoutDomain("$episodeLink/$i")
                    name = "Episodio $i"
    ...
    // Unused methods from ParsedAnimeHttpSource
